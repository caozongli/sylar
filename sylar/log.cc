...
struct LogDefine {
    ...
    friend std::ostream & operator << (std::ostream &out, const LogDefine& s){
        out << "LogDefine " << s.name << std::endl;
        return out;
    }
};
        ...
        if(n["appenders"].IsDefined()) {
            ...
            for(size_t x = 0; x < n["appenders"].size(); ++x) {
                ...
                if(type == "FileLogAppender") {
                    ...
                    lad.level = LogLevel::FromString(n["level"].IsDefined() ? n["level"].as<std::string>() : "");
...
struct LogIniter* __log_init = new LogIniter();
